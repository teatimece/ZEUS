...
<%@ page import="com.koreait.admin.block.BlockDTO" %>
...
<%
	...
	List<MemberDTO> memberList = dao.blockMember(start, pagePerCount);
	List<BlockDTO> blockList = dao.blockDetail(start, pagePerCount);
...
<body>
    <div id="container">
            ...
            <ul>
                <li>
                    ...
                    <ul>
                        ...
                        <li class="title"><a href="../member/mem_block.jsp">- 계정차단회원관리</a></li>
        ...
        <div class="contents">
            <h1><img src="../images/arrow.png">계정차단회원관리</h1>
            <div id="mem_withdrawal">
                <div class="mem_search">
                    <form method="post" action="./mem_block_search.jsp">
	                    <p>
...
<%								
						for(int i=0; i<memberList.size(); i++){
							...
							String store = "";
							if(memberList.get(i).getStore()==null){
								store = "상점"+Integer.toString(memberList.get(i).getIdx())+"호";                            	
                            }else{
                            	store = memberList.get(i).getStore();
                            }
%>
						<tr>
                            <td><%= memberList.get(i).getIdx() %></td>
                            <td><a href="./mem_detail.jsp?m_idx=<%=memberList.get(i).getIdx()%>"><%=memberList.get(i).getUsername() %>(<%=store %>)</a></td>
                            <td><%= proCnt %></td>
                            <td><%= memberList.get(i).getJoindate() %></td>
<%
							blockList.get(i);
%>
							<td><%= blockList.get(i).getBlockdate() %></td>
							<td><%= blockList.get(i).getBlockReason() %></td>
							<td><input type="button" value="해제" class="unblock" onclick="location.href='./mem_unblock_ok.jsp?b_memidx=<%= memberList.get(i).getIdx() %>'"></td>
                            <td><input type="button" value="삭제" class="userDel" onclick="location.href='./mem_delete_ok.jsp?m_idx=<%= memberList.get(i).getIdx() %>&&page=mem_block'"></td>
						</tr>
<%
						}
...
<%
				for(int i=1;i<=pageNums;i++){
					out.print("<a href='mem_block.jsp?pageNum=" + i + "'>[" + i + "]</a>" + "&nbsp;&nbsp;");
				}                  }else{
					%>
