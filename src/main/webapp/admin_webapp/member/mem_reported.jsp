...
<%
	...
	List<ReportDTO> reportList = dao.selectMember(start, pagePerCount);
...
%>
                        <tr>
                            <td><%= report.getMemidx() %></td>
                            <td class="redtxt"><%= memname %>(<%= memstore %>)</a></td>
                            <td><%= report.getReason() %></td>
                            <td><%= product %></td>
                            <td><%= report.getRegdate() %></td>
                            <td><%= reportername %>(<%= reporterstore %>)</td>
...
<%
				for(int i=1;i<=pageNums;i++){
					out.print("<a href='mem_reported.jsp?pageNum=" + i + "'>[" + i + "]</a>" + "&nbsp;&nbsp;");
				}
			}else{
%>
